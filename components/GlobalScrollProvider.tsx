...
const GlobalScrollContext = createContext<GlobalScrollContextType | undefined>(undefined);

export const useGlobalScroll = () => {
  const context = useContext(GlobalScrollContext);
  if (!context) {
    throw new Error('useGlobalScroll must be used within a GlobalScrollProvider');
  }
  return context;
};
...
export default function GlobalScrollProvider({ children }: GlobalScrollProviderProps) {
  ...
  const totalScreens = PAGE_CONFIGS.reduce((sum, page) => sum + page.screens, 0);
  ...
  useEffect(() => {
    ...
    const isScrollablePage = SCROLLABLE_PAGES.includes(pathname);

// Complete JCSS Team Data - All 22 Members
// This file contains comprehensive data for all team members

export const teamMembers = [
  {
    id: 1,
    name: "MAHAVEER C JAIN",
    image: "/images/team/01 MAHAVEER JAIN.jpg",
    position: "Managing Partner",
    description:
      "Fellow member of ICAI and Founder member of JCSS. Specialised in Transaction Advisory, Transfer Pricing Regulations, Cross border tax structuring, Expatriate Tax Planning and Litigation.",
    specializations: [
      "Cross Border Tax Structuring",
      "Transfer Pricing",
      "Litigation",
      "Expatriate Taxation",
    ],
    academic: ["FCA"],
    linkedin: "https://linkedin.com/in/mahaveer-jain",
  },
  {
    id: 2,
    name: "JATIN CHRISTOPHER",
    image: "/images/team/02 JATIN CHRISTOPHER.jpg",
    position: "Partner",
    description:
      "Fellow member of ICAI & ICMAI and Founder Member of JCSS. Specialised in GST, SPAs (Sale and Purchase Agreements) and PDAs (Product Distribution Agreements) of global entities. Faculty & Senior Resource member of the ICAI on Indirect Taxes and advised on clarifications on GST matters at various forums.",
    specializations: [
      "Cross Border Tax Structuring",
      "Transfer Pricing",
      "Litigation",
      "Expatriate Taxation",
    ],
    academic: ["FCA", "FCMA"],
    linkedin: "https://linkedin.com/in/jatin-christopher",
  },
  {
    id: 3,
    name: "SRIKANTH BALAKRISHNAN",
    image: "/images/team/03 SRIKANTH BALAKRISHNAN.jpg",
    position: "Senior Manager",
    description:
      "Fellow member of ICAI and Founder Member of JCSS. Specialised in Indirect Taxation, Cross Border Investments into India across various industries. He is also part of JCSS Singapore & Japan practices.",
    specializations: [
      "GST",
      "Customs",
      "Cross Border Investment",
      "Litigation",
    ],
    academic: ["FCA"],
    linkedin: "https://linkedin.com/in/srikanth-balakrishnan",
  },
  {
    id: 4,
    name: "SRIKANTH SRINIVASAN",
    image: "/images/team/04 SRIKANTH SRINIVASAN.jpg",
    position: "Manager",
    description:
      "Fellow member of ICAI and Founder Member of JCSS. Specialised in Funding Options, Capital Structure, FEMA Regulations, Internal Audit and SOX compliance. He is also part of JCSS Singapore & Japan practices.",
    specializations: [
      "Audit and Assurance",
      "FEMA",
      "Companies Act",
      "Start-up Advisory",
      "Mergers & Acquisitions",
      "Joint Venture & Valuation",
      "SOX & Risk Advisory",
    ],
    academic: ["FCA"],
    linkedin: "https://linkedin.com/in/srikanth-srinivasan",
  },
  {
    id: 5,
    name: "SRINIVAS P R",
    image: "/images/team/05 SRINIVAS P R.jpg",
    position: "Senior Manager",
    description:
      "Fellow member of ICAI & ICMA. Specialised in Accounting (Indian GAAP), Audit compliance and, ERP systems implementation.",
    specializations: [
      "Audit and Assurance",
      "Companies Act",
      "Indian GAAP",
      "ERP Implementation",
    ],
    academic: ["FCA", "CMA"],
    linkedin: "https://linkedin.com/in/srinivas-pr",
  },
  {
    id: 6,
    name: "SHRINIVAS KULKARNI",
    image: "/images/team/06 SHRINIVAS KULKARNI.jpg",
    position: "Manager",
    description:
      "Law Graduate, Accounting and Auditing Professional. Specialised in Financial Accounting and Enterprise Services & Solutions.",
    specializations: ["Regulatory Compliance", "Financial Accounting"],
    academic: ["LLB"],
    linkedin: "https://linkedin.com/in/shrinivas-kulkarni",
  },
  {
    id: 7,
    name: "M SUBASH",
    image: "/images/team/SUBASH.jpg",
    position: "Fractional CFO",
    description:
      "Fellow Chartered Accountant and Cost and Management Accountant with over 35 years of experience across different industries ranging from manufacturing and IT Hardware to IT Services across India and Malaysia. Involved in Business Transformation, Risk Management, and Regulatory Leadership. Most notably, served as a Board Member at Dell India for over a decade, driving decision-making at the highest levels.",
    specializations: [
      "Fractional CFO Services",
      "Business Transformation",
      "Risk Management",
      "Regulatory Leadership",
      "Financial Management",
    ],
    academic: ["FCA", "CMA"],
    linkedin: "https://www.linkedin.com/in/m-subash-blr/",
  },
  {
    id: 8,
    name: "VINUTA U V",
    image: "/images/team/07 VINUTA U V.jpg",
    position: "Senior Associate",
    description:
      "Fellow member of ICSI; Board member of JCSS. Specialised in Transaction Advisory and Corporate Secretarial services.",
    specializations: [
      "Indian Companies Act",
      "FEMA Advisory",
      "Inbound & Outbound Advisory",
    ],
    academic: ["ACS"],
    linkedin: "https://linkedin.com/in/vinuta-uv",
  },
  {
    id: 9,
    name: "PRASHANTH PRABHU G",
    image: "/images/team/08 PRASHANTH PRABHU G.jpg",
    position: "Associate Manager",
    description:
      "Fellow member of ICAI and, part of Audit & Assurance and Tax Compliance practice of JCSS. Specialised in Group companies's IFRS compliance, ICFR and Internal Audits.",
    specializations: ["Audit and Assurance", "Companies Act", "IFRS", "ICFR"],
    academic: ["FCA"],
    linkedin: "https://linkedin.com/in/prashanth-prabhu",
  },
  {
    id: 10,
    name: "V.S.HARISH",
    image: "/images/team/09 V.S.HARISH.jpg",
    position: "Senior Associate",
    description:
      "Fellow member of ICAI and, part of Audit & Assurance and Tax Compliance practice of JCSS. Specialised in Group companies's IFRS compliance, ICFR and Internal Audits.",
    specializations: ["Income Tax", "Auditing", "Business Consulting"],
    academic: ["FCA"],
    linkedin: "https://linkedin.com/in/vs-harish",
  },
  {
    id: 11,
    name: "NIRNAJAN N K",
    image: "/images/team/10 NIRANJAN NK.jpg",
    position: "Associate",
    description:
      "Associate member of ICAI. Specialised in Audit and Assurance. Partner of Audit & Assurance and Advisory division at JCSS",
    specializations: [
      "Audit and Assurance",
      "ICFR Implementation and reporting",
    ],
    academic: ["FCA"],
    linkedin: "https://linkedin.com/in/niranjan-nk",
  },
  // {
  //   id: 12,
  //   name: "NAGARAJA K S",
  //   image: "/images/team/11 NAGARAJA K S.jpg",
  //   position: "Senior Associate",
  //   description: "Expert in corporate advisory and business restructuring with extensive experience in mergers and acquisitions.",
  //   specializations: [
  //     "Corporate Advisory",
  //     "Business Restructuring",
  //     "Mergers & Acquisitions",
  //     "Due Diligence"
  //   ],
  //   academic: ["CA", "MBA"],
  //   linkedin: "https://linkedin.com/in/nagaraja-ks"
  // },
  {
    id: 13,
    name: "MADHUSOODHAN HEGDE",
    image: "/images/team/12 MADHUSOODAN HEGDE.jpg",
    position: "Manager",
    description:
      "Fellow member of ICAI. Specialised in Indirect Taxation / GST.",
    specializations: ["Customs", "GST Litigation"],
    academic: ["FCA"],
    linkedin: "https://linkedin.com/in/madhusoodan-hegde",
  },
  {
    id: 14,
    name: "SURESH K G",
    image: "/images/team/13 SURESH K G.jpg",
    position: "Senior Associate",
    description:
      "A pioneer in business between Japan and India for over 30 years. He moved to Japan during his university days and established and managed a joint venture with a large Japanese company. He collaborates with top Japanese companies to support investment and new business development in India. Using his know-how and network for doing business with Japan, he innovated his model of collaboration and contributed greatly to the prosperity of business between Japan and India.",
    specializations: [
      "Technology Venture Strategy",
      "Market Development",
      "Intelligence",
    ],
    academic: ["Ph.D"],
    linkedin: "https://linkedin.com/in/suresh-kg",
  },
  {
    id: 15,
    name: "LOGANATHAN ANANDAN",
    image: "/images/team/14 LOGANATHAN ANANDAN.jpg",
    position: "Associate",
    description:
      "Loganathan comes with about 2 decades of consulting cum industry experience across Southeast Asia, Africa, and USA. Specialized in Risk Advisory, SOX (Sarbanes Oxley), Information Systems / ERP Audits, Singapore & Indonesian compliance solutions. Co-founder of JCSS southeast Asia.",
    specializations: [
      "Risk Advisory",
      "Information Systems & ERP Audit",
      "SOX",
      "Singapore & Indonesian Compliance",
    ],
    academic: [
      "FIPA (Australia)",
      "FAIA (UK)",
      " ACA (India)",
      "CFE",
      "CISA & CDPSE",
      "CRMA",
      "PTP (Singapore Chartered Tax Professionals)",
    ],
    linkedin: "https://linkedin.com/in/loganathan-anandan",
  },
  {
    id: 16,
    name: "YASHVANT NIJAGAL",
    image: "/images/team/15 YASHVANT NIJAGAL.jpg",
    position: "Senior Associate",
    description:
      "A seasoned Chartered Accountant in the field of Finance with over a decade’s experience in Startup Consulting. Yashvant is passionate about contributing to the success of businesses by combining financial acumen with technological solutions. Yashvant positions himself as a trusted advisor in the startup ecosystem and a trailblazer in enterprise technology solutions",
    specializations: [
      "Startup Advisory",
      "Business Consulting",
      "Technology Integration",
    ],
    academic: ["CA"],
    linkedin: "https://linkedin.com/in/yashvant-nijagal",
  },
  {
    id: 17,
    name: "KIRAN KUMAR",
    image: "/images/team/16 KIRAN KUMAR.jpg",
    position: "Associate",
    description:
      "Kiran Kumar, a seasoned Audit and Assurance professional with 13+ years of experience, holds qualifications including B.Com, FCA, CS, and ACCA Finalist. As an Associate Partner at JCSS, he leads domestic and international teams, specializing in statutory audit, IFC documentation, and tailored solutions across IT, Manufacturing, Real Estate, Fintech, and other sectors. Committed to transparency and professionalism, he ensures compliance and fosters long-term client relationships.",
    specializations: [
      "IT sector",
      "Manufacturing sector",
      "Real Estate",
      "Fintech",
      "Trading and Other Services sectors",
    ],
    academic: ["B.Com", "FCA", "CS"],
    linkedin: "https://linkedin.com/in/kiran-kumar",
  },
  {
    id: 18,
    name: "SUBHASH MEHTA",
    image: "/images/team/17 SUBHASH MEHTA.jpg",
    position: "Manager",
    description:
      "Subhash Mehta is a Member of Institute of Chartered Accountants of India and Institute of Company Secretaries of India. Head of Direct Taxation for the JCSS Bangalore & Pune Office.CA Subhash Mehta has over 12 years of varied experience in providing corporate tax advisory, transaction advisory, statutory compliance, International taxation, tax litigation and transfer pricing.",
    specializations: [
      "Corporate tax advisory",
      "Transaction advisory",
      "Statutory compliance",
      "International taxation",
      "Tax litigation",
      "Transfer pricing",
    ],
    academic: ["CA", "CS"],
    linkedin: "https://linkedin.com/in/subhash-mehta",
  },
  {
    id: 19,
    name: "PADIKKAL VEETTIL RAJESH",
    image: "/images/team/18 PADIKKAL VEETTIL RAJESH.jpg",
    position: "Senior Associate",
    description:
      "Rajesh Padikkal Veettil, M.Com, FCA, PGDCA, a commerce graduate who completed his Chartered Accountancy in October 2006, is a seasoned corporate tax professional with over 17 years of experience. His expertise includes litigation support, transfer pricing, cross-border transactions, tax optimization, international tax treaties, mergers and acquisitions, and tax audits.",
    specializations: [
      "Transfer pricing compliance and benchmarking solutions.",
      "Cross-border transaction structuring.",
      "Tax optimization consulting and Transaction planning.",
      "International tax treaty consulting.",
      "Mergers and acquisitions.",
      "Tax audits, consulting, and certifications.",
      "Litigation support services, encompassing representation before various tax authorities and appellate bodies",
    ],
    academic: ["M Com", "FCA", "PGDCA"],
    linkedin: "https://linkedin.com/in/padikkal-veettil-rajesh",
  },
  {
    id: 20,
    name: "MANEESH AGARWAL",
    image: "/images/team/19 MANEESH AGARWAL.jpg",
    position: "Associate Manager",
    description:
      "Maneesh Agarwal, a Chartered Accountant and Partner at JCSS. Maneesh has been a driving force in the field of professional service specializing in taxation.As the head of the Ahmedabad office, Maneesh leads with a strategic vision, aligning the firm with the dynamic needs of clients and the evolving taxation landscape.",
    specializations: [
      "Tax Litigation",
      "Corporate Tax Advisory",
      "Transaction Advisory",
      "International Taxation",
      "Transfer Pricing",
    ],
    academic: ["CA"],
    linkedin: "https://linkedin.com/in/maneesh-agarwal",
  },
  {
    id: 21,
    name: "VIJAYACHANDRA SHETTY",
    image: "/images/team/20 VIJAYACHANDRA SHETTY.jpg",
    position: "Senior Associate",
    description:
      "Chartered Accountant and Fellow Member of JCSS. Possesses expertise in Accounting, Financial Reporting, Payroll Management, and Regulatory Compliance.",
    specializations: [
      "Accounting",
      "Financial reporting",
      "Payroll management",
      "Regulatory compliances",
    ],
    academic: ["CA"],
    linkedin: "https://linkedin.com/in/vijayachandra-shetty",
  },
  {
    id: 22,
    name: "SATYAJIT JOSHI",
    image: "/images/team/21 SATYAJIT JOSHI.jpg",
    position: "Associate",
    description:
      "Satyajit is an Associate Member of ICSI and has over 18 years’ experience in Corporate Advisory Services. Satyajit is an Associate Director in JCSS and specialises in Corporate Secretarial services and Transaction Advisory services.",
    specializations: [
      "Indian Companies Act",
      "FEMA Advisory",
      "Inbound and Outbound Advisory",
      "M&A",
    ],
    academic: ["ACS"],
    linkedin: "https://linkedin.com/in/satyajit-joshi",
  },
  {
    id: 23,
    name: "SHAILESH BAPAT",
    image: "/images/team/22 SHAILESH BAPAT.jpg",
    position: "Senior Manager",
    description:
      "Shailesh is a seasoned Chartered Accountant with over 15 years of experience specializing in Indirect Tax. He has advised multinational corporations in IT/ITES, Fintech, manufacturing, and trading industries. His expertise includes Customs, SEZ, GST, and Foreign Trade Policy. Shailesh is recognized for providing strategic guidance, optimizing tax positions, and ensuring compliance with a dynamic regulatory landscape.",
    specializations: [
      "Tax advisory",
      "Transaction structuring",
      "Regulatory compliance",
      "Litigation",
      "Due Diligence",
    ],
    academic: ["CA"],
    linkedin: "https://linkedin.com/in/shailesh-bapat",
  },
];
